app.controller('DashBoard_OperController', function ($scope, CIR_OperationService) {
    ...
    $scope.TypeOfCollectionSMPData = "Monthly";
    $scope.IsMyPending = "0";
    $scope.Add = "1";
    $scope.View = "1";
    $scope.Update = "1";
    ...
    function fnSetAccess() {
        ...
        promisePost.then(function (pl) {
            ...
            $scope.Delete = pl.data.result.Delete;
    ...
    $scope.GetDetailsForSMPData = function () {
        var Data = fnDropDownDataSMPData($("#ddlTypeForSMPData").val(), $("#spnCurrentFORSMPData").text(), $("#spnNextFORSMPData").text(), $("#spnPreviousFORSMPData").text(), "0");
        var promisePost = CIR_OperationService.postCommonService(GetDashBoardDetailsForSMPDataUrl, Data);
        promisePost.then(function (pl) {
            $scope.DashBoardArrayFORSMPData = pl.data.result;
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            console.log("Error in getting the details for SMP Data : " + err);

        });
    };
    $scope.GetDetailsForSMPData();

    $scope.GetSMPDetailsData = function (title) {
        debugger
        var statusType = "0";
        if (title.toLowerCase() == "mypending")
            statusType = "0"
        if (title.toLowerCase() == "completed")
            statusType = "1"
        var Data = fnDropDownDataSMPData($("#ddlTypeForSMPData").val(), $("#spnCurrentFORSMPData").text(), $("#spnNextFORSMPData").text(), $("#spnPreviousFORSMPData").text(), statusType);
        fnLoadGif();
        var promisePost = CIR_OperationService.postCommonService(GetSMPDataDashboardDetailsUrl, Data);
        promisePost.then(function (pl) {
            $scope.DashBoardArrayFORSMPDataDetails = pl.data.result;
            fnRemoveGif();
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            console.log("Error in getting the details for SMP Data Details : " + err);

        });
    }

    function fnDropDownDataSMPData(ddlValue, current, next, previous, statusType) {
        var currentDate = new Date();
        var monthNames = ["Sam", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        var twoDigitMonth = ((parseInt(currentDate.getMonth() + 1)).toString().length != 1) ? (parseInt(currentDate.getMonth() + 1)) : '0' + (parseInt(currentDate.getMonth() + 1));
        var MonthValue = current;
    ...
    $scope.LoadSMPData = function (row) {
        debugger
        var Data = {
            smpDataUploadId: row.SUId,
            level: row.Level
        };
        localStorage.setItem("SUId", row.SUId);
        localStorage.setItem("Levels", row.Level);
        localStorage.setItem("FileName", row.Name);
        window.location.href = SMPDataUrl;
    }
