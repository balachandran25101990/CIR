...
app.controller('RoleAccess_OperController', function ($scope, CIR_OperationService, $filter) {
    ...
    function LoadSMPAccessGrid() {
        fnLoadGif();
        var RoleAccessModel = {
            Rleid: $scope.SMPRoleId,
            PgeId: "0",
            ViewType: "1"
        };
        var promisePost = CIR_OperationService.postCommonService(GetRoleUrl, RoleAccessModel);
        promisePost.then(function (pl) {
            $scope.SMPAccessGridArray = pl.data.result;
            fnRemoveGif();
        }, function (err) {
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
    ...
    $scope.SetSMPAccess = function () {
        $("html, body").animate({ scrollTop: 0 }, "slow");
        var Access = "";
        var row;
        for (var i = 0; i < $scope.SMPAccessGridArray.length; i++) {
            Access += "<data>"
            row = $scope.SMPAccessGridArray[i]
            Access += "<RAId>" + row.RAId + "</RAId>";
            Access += "<PGEId>" + row.PgeID + "</PGEId>";
            Access += "<Access>" + row.View + "</Access>";
            Access += "<Add>" + row.Add + "</Add>";
...
app.controller('Turbine_OperController', function ($scope, CIR_OperationService, $filter, $interval) {
    ...
    function LoadTurbineGrid() {
        ...
        var promisePost = CIR_OperationService.postCommonService(GetTurbineEntryUrl, TurbineModel);
        promisePost.then(function (pl) {

            $scope.TurbineGridArray = pl.data.TurbineDetail;
            $scope.TurbineArrayLength = pl.data.TurbineDetail.length;
            $scope.TurbineGridArrayFiltered = $scope.TurbineGridArray;
            fnRemoveGif();
        }, function (err) {
            fnRemoveGif();
...
app.controller('TurbineVersionUpdate_OperController', function ($scope, CIR_OperationService, $filter, $interval) {
    ...
    function IsTabAvailable() {
        ...
        if ($scope.Add) {
            ...
            $('#tablist1-panel2').addClass('responsive-tabs__panel responsive-tabs__panel--active responsive-tabs__panel--closed-accordion-only');
        }
        else {
        ...
        if ($scope.Update) {
            ...
            $('#tablist1-panel3').addClass('responsive-tabs__panel responsive-tabs__panel--active responsive-tabs__panel--closed-accordion-only');
        }
        else {
            $('#tablist1-tab3').css('display', 'none');
        }
        if ($scope.Delete) {
            $('#tablist1-tab4').addClass('responsive-tabs__list__item responsive-tabs__list__item--active');
            $('#tablist1-panel4').addClass('responsive-tabs__panel responsive-tabs__panel--active responsive-tabs__panel--closed-accordion-only');
        }
        else {
            $('#tablist1-tab4').css('display', 'none');
        }
...
app.controller('SMPData_OperController', function ($scope, CIR_OperationService, $filter, $interval) {
    LoadStatusOrAssign(3, 1);
    LoadStatusOrAssign(3, 2);
    LoadStatusOrAssign(3, 3);
    LoadStatusOrAssign(3, 4);
    LoadStatusOrAssign(2, 1);
    $scope.Add = true;
    $scope.View = true;
    $scope.Update = true;
    $scope.Delete = true;
    $scope.Download = true;
    $scope.SMPDataRemarks = "";
    $scope.SMPDataFileName = "";
    $scope.isEditSMPData = true;
    $scope.isEditVerify = true;
    $scope.IsSaveEditable = true;
    $scope.IsSaveEditableVerify = true;
    $scope.StatusVerify = "0";
    $scope.AssignedToVerify = "0";
    $scope.IsSaveEditableApprove = true;
    $scope.StatusApprove = "0";
    $scope.AssignedToApprove = "0";
    $scope.isEditApprove = true;
    $scope.IsSaveEditablePost = true;
    $scope.StatusPost = "0";
    $scope.AssignedToPost = "0";
    $scope.isEditPost = true;
    $scope.IsAdmin = false;
    LoadWTGType();
    $("#btnCommentsApprove").attr("disabled", "disabled");
    ...
    $scope.LoadPageAccess = function () {
        var Data = {
            pageName: "SMP Data Plan Vs Actual Status"
        };
        ...
        promisePost.then(function (pl) {
            ...
            IsTabAvailable();
            var DashboardSUId = "0";
            DashboardSUId = localStorage.getItem("SUId");
            var DashboardLevel = "1";
            DashboardLevel = localStorage.getItem("Levels");
            var DashboardName = "";
            DashboardName = localStorage.getItem("FileName");
            debugger
            if (DashboardSUId != null) {
                if (DashboardLevel == "1") {
                    $("#txtFileNo").val(DashboardSUId);
                    $scope.SUId = DashboardSUId;
                    $scope.SUNo = DashboardSUId;
                    $("#txtFileNameManualSMPData").val(DashboardName);
                    $("#hdnFileNameManualSMPData").val(DashboardSUId);
                    $("#tablist1-tab1").click();
                }
                if (DashboardLevel == "2") {
                    $scope.FileNameManualVerify = DashboardName;
                    $("#txtFileNameManualVerify").val(DashboardName);
                    $("#hdnFileNameManualVerify").val(DashboardSUId);
                    $("#tablist1-tab2").click();
                }
                if (DashboardLevel == "3") {
                    $scope.FileNameManualApprove = DashboardName;
                    $("#txtFileNameManualApprove").val(DashboardName);
                    $("#hdnFileNameManualApprove").val(DashboardSUId);
                    $("#tablist1-tab3").click();
                }
                if (DashboardLevel == "4") {
                    $scope.FileNameManualPost = DashboardName;
                    $("#txtFileNameManualPost").val(DashboardName);
                    $("#hdnFileNameManualPost").val(DashboardSUId);
                    $("#tablist1-tab4").click();
                }
                LoadSMPDataExcel(DashboardSUId, DashboardLevel);
                localStorage.removeItem("SUId");
                localStoarge.removeItem("Levels");
                localStorage.removeItem("FileName");
            }
            else
                LoadSMPDataExcel("0", 1);
    ...
    function IsTabAvailable() {
        if ($scope.Add) {
            $('#tablist1-tab1').addClass('responsive - tabs__list__item');
            $('#tablist1-panel1').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $("#tablist1-tab1").click();
        }
        else {
            $('#tablist1-tab1').css('display', 'none');
        ...
        if ($scope.Update) {
            $('#tablist1-tab2').addClass('responsive - tabs__list__item');
            $('#tablist1-panel2').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $("#tablist1-tab2").click();
        }
        else {
            $('#tablist1-tab2').css('display', 'none');
        ...
        if ($scope.Delete) {
            $('#tablist1-tab3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $("#tablist1-tab3").click();
        }
        else {
            $('#tablist1-tab3').css('display', 'none');
        ...
        if ($scope.Download) {
            $('#tablist1-tab4').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $("#tablist1-tab4").click();
        }
        else {
            $('#tablist1-tab4').css('display', 'none');
    ...
    $scope.UploadSMPDataUpdateExcel = function () {
        ...
        promisePost.then(function (pl) {
            ...
            if ($scope.Message.Clear == "True") {
                ...
                $scope.SMPDataFileName = $scope.Message.FileName;
                $("#btnSaveSMPData").removeAttr("disabled");
                $("#btnCancelSMPData").removeAttr("disabled");
                $("#txtSMPDataRemarks").removeAttr("disabled");
                $scope.isEditSMPData = true;
                //LoadPostFileName();
                //$("#btnSave").removeAttr("disabled");
            }
                ...
                $scope.SMPDataExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.new_account = false;
                $("#btnSaveSMPData").attr("disabled", "disabled");
                $("#btnCancelSMPData").attr("disabled", "disabled");
                $("#txtSMPDataRemarks").attr("disabled", "disabled");
            }
    ...
    function LoadSMPDataExcel(SUNo, level) {
        $('#ExcelFileSMP').val("");
        ...
        promisePost.then(function (pl) {
            debugger
            $scope.IsAdmin = pl.data.result.headerdetails.IsAdmin;
            if (level == 1) {
                $scope.SMPDataExcelHeader = pl.data.result.headerdetails;
                $scope.FileNameManualSMPData = $scope.SMPDataExcelHeader.ManualFileName;
                ...
                $scope.StatusSMPDataName = $scope.SMPDataExcelHeader.Status;
                $scope.AssignedToSMPDataName = $scope.SMPDataExcelHeader.EmpName;
                $scope.SMPDataExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.IsSaveEditable = $scope.SMPDataExcelHeader.IsSaveEditable;
                $scope.AssignedToSMPData = $scope.SMPDataExcelHeader.EmpId;
                $scope.StatusSMPData = $scope.SMPDataExcelHeader.StatusId;
                if (SUNo != "0")
                    $("#btnCommentsSMPData").removeAttr("disabled");
                else
                    $("#btnCommentsSMPData").attr("disabled", "disabled");
                if ($scope.IsSaveEditable) {
                    $("#btnSaveSMPData").removeAttr("disabled");
                    $("#btnCancelSMPData").removeAttr("disabled");
                    $("#txtSMPDataRemarks").removeAttr("disabled");
                    $("#btnUpload").removeAttr("disabled");
            ...
            if (level == 2) {
                $scope.VerifyExcelHeader = pl.data.result.headerdetails;
                $scope.SUNoVerify = $scope.VerifyExcelHeader.SUNo;
                $scope.SUIdVerify = $scope.VerifyExcelHeader.SUId;
                $scope.VerifyExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.StatusVerifyName = $scope.VerifyExcelHeader.Status;
                $scope.AssignedToVerifyName = $scope.VerifyExcelHeader.EmpName;
                $scope.IsSaveEditableVerify = $scope.VerifyExcelHeader.IsSaveEditable;
                $scope.AssignedToVerify = $scope.VerifyExcelHeader.EmpId;
                $scope.StatusVerify = $scope.VerifyExcelHeader.StatusId;
                if ($scope.StatusVerify == "0")
                    $scope.LoadAssignDetailsToVerify();
                $("#txtVerifyRemarks").removeAttr("disabled");
                if (SUNo != "0")
                    $("#btnCommentsVerify").removeAttr("disabled");
                else
                    $("#btnCommentsVerify").attr("disabled", "disabled");
                if ($scope.IsSaveEditable) {
                    $("#btnSaveVerify").removeAttr("disabled");
                    $("#txtVerifyRemarks").removeAttr("disabled");
                    $("#btnUpload").removeAttr("disabled");
                }
                else {
                    $("#btnSaveVerify").attr("disabled", "disabled");
                    $("#txtVerifyRemarks").attr("disabled", "disabled");
                    $("#btnUpload").attr("disabled", "disabled");
                }
            }
            if (level == 3) {
                $scope.ApproveExcelHeader = pl.data.result.headerdetails;
                $scope.SUNoApprove = $scope.ApproveExcelHeader.SUNo;
                $scope.SUIdApprove = $scope.ApproveExcelHeader.SUId;
                $scope.ApproveExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.StatusApproveName = $scope.ApproveExcelHeader.Status;
                $scope.AssignedToApproveName = $scope.ApproveExcelHeader.EmpName;
                $scope.IsSaveEditableApprove = $scope.ApproveExcelHeader.IsSaveEditable;
                $scope.AssignedToApprove = $scope.ApproveExcelHeader.EmpId;
                $scope.StatusApprove = $scope.ApproveExcelHeader.StatusId;
                $("#txtApproveRemarks").removeAttr("disabled");
                if ($scope.StatusApprove != "0")
                    $scope.LoadAssignDetailsToApprove();
                if (SUNo != "0")
                    $("#btnCommentsApprove").removeAttr("disabled");
                else
                    $("#btnCommentsApprove").attr("disabled", "disabled");
                if ($scope.IsSaveEditable) {
                    $("#btnSaveApprove").removeAttr("disabled");
                    $("#txtApproveRemarks").removeAttr("disabled");
                }
                else {
                    $("#btnSaveApprove").attr("disabled", "disabled");
                    $("#txtApproveRemarks").attr("disabled", "disabled");
                }
            }
            if (level == 4) {
                $scope.PostExcelHeader = pl.data.result.headerdetails;
                $scope.SUNoPost = $scope.PostExcelHeader.SUNo;
                $scope.SUIdPost = $scope.PostExcelHeader.SUId;
                $scope.PostExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.StatusPostName = $scope.PostExcelHeader.Status;
                $scope.AssignedToApproveName = $scope.PostExcelHeader.EmpName;
                $scope.IsSaveEditablePost = $scope.PostExcelHeader.IsSaveEditable;
                $scope.AssignedToPost = $scope.PostExcelHeader.EmpId;
                $scope.StatusPost = $scope.PostExcelHeader.StatusId;
                $("#txtPostRemarks").removeAttr("disabled");
                $("#ddlAssignedToPost").attr("disabled", "disabled");
                
                    //$scope.LoadAssignDetailsToPost();
                if (SUNo != "0")
                    $("#btnCommentsPost").removeAttr("disabled");
                else
                    $("#btnCommentsPost").attr("disabled", "disabled");
                if ($scope.IsSaveEditable) {
                    ...
                    $("#txtPostRemarks").attr("disabled", "disabled");
                }
            }
            if (level == 0) {
                $scope.AdminExcelHeader = pl.data.result.headerdetails;
                $scope.SUNoPost = $scope.AdminExcelHeader.SUNo;
                $scope.SUIdPost = $scope.AdminExcelHeader.SUId;
                $scope.AdminExcelGridArray = pl.data.result.SMPDataExcelDetails;
                $scope.StatusAdmin = $scope.AdminExcelHeader.Status;
                $scope.AssignToAdmin = $scope.AdminExcelHeader.EmpName;
                if (SUNo != "0")
                    $("#btnCommentsAdmin").removeAttr("disabled");
                else
                    $("#btnCommentsAdmin").attr("disabled", "disabled");
            }
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
    ...
    $scope.FileNoChange = function (keyEvent, level) {
        $scope.UpdateOnly = false;
        //if (level == 1) {
        if (keyEvent.which === 13) {
            $scope.SMPDataExcelGridArray = [];
            LoadSMPDataExcel($scope.SUNo, 1);
            $("#btnSaveSMPData").attr("disabled", "disabled");
            $("#btnCancelSMPData").attr("disabled", "disabled");
            $("#txtFileNameManualSMPData").attr("disabled", "disabled");
            $("#txtSMPDataRemarks").attr("disabled", "disabled");
            $scope.isEditSMPData = false;
        }
        //}
        //if (level == 2) {
        //    if (keyEvent.which === 13) {
        //        $scope.SMPDataExcelGridArray = [];
        //        LoadSMPDataExcel($scope.SUNo);
        //        $("#btnSaveVerify").attr("disabled", "disabled");
        //        $("#btnCancelVerify").attr("disabled", "disabled");
        //        $("#txtVerifyRemarks").attr("disabled", "disabled");
        //        $scope.isEditVerify = false;
        //    }
        //}
    };
    ...
    $scope.SubmitSMPDataExcel = function () {
        if ($.trim($scope.SMPDataRemarks) == "") {
            SetErrorMessage("Ensure comments!");
            $("#txtSMPDataRemarks").focus();
            return false;
        }
        if ($scope.StatusSMPData == "0" || $scope.StatusSMPData == null || $scope.StatusSMPData == null) {
            SetErrorMessage("Ensure Status!");
            $("#ddlStatusSMPData").focus();
            return false;
        }
        if ($scope.AssignedToSMPData == "0" || $scope.AssignedToSMPData == null || $scope.AssignedToSMPData == null) {
            SetErrorMessage("Ensure Assign To!");
            $("#ddlAssignedToSMPData").focus();
            return false;
        }
        fnLoadGif();
        var InputModel = {
            SUId: $scope.SUNo,
            Remarks: $scope.SMPDataRemarks,
            FileName: $scope.SMPDataFileName,
            smpData: JSON.stringify($scope.SMPDataExcelGridArray),
            fileNameManual: $("#txtFileNameManualSMPData").val(),
            comments: $scope.SMPDataRemarks,
            status: $scope.StatusSMPData,
            assignTo: $scope.AssignedToSMPData
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataExcelSubmit, InputModel);
        promisePost.then(function (pl) {
            $scope.Message = pl.data.result.message;
            $scope.SUId = "0";
            if ($scope.Message.Clear == "True") {
    ...
    $scope.GetFileNameDetails = function (level) {
        if (level == 1) {
            if ($("#hdnFileNameManualSMPData").val() != "0") {
                $scope.SMPDataExcelGridArray = [];
                LoadSMPDataExcel($("#hdnFileNameManualSMPData").val(), level);
                $("#btnSaveSMPData").attr("disabled", "disabled");
        ...
        if (level == 2) {
            if ($("#hdnFileNameManualVerify").val() != "0") {
                $scope.VerifyExcelGridArray = [];
                LoadSMPDataExcel($("#hdnFileNameManualVerify").val(), level);
                $("#btnSaveVerify").attr("disabled", "disabled");
                $("#txtVerifyRemarks").attr("disabled", "disabled");
            }
        }
        debugger
        if (level == 3) {
            if ($("#hdnFileNameManualApprove").val() != "0") {
                $scope.ApproveExcelGridArray = [];
                LoadSMPDataExcel($("#hdnFileNameManualApprove").val(), level);
                $("#btnSaveApprove").attr("disabled", "disabled");
                $("#txtApproveRemarks").attr("disabled", "disabled");
            }
        }
        if (level == 4) {
            if ($("#hdnFileNameManualPost").val() != "0") {
                $scope.PostExcelGridArray = [];
                LoadSMPDataExcel($("#hdnFileNameManualPost").val(), level);
                $("#btnSavePost").attr("disabled", "disabled");
                $("#txtPostRemarks").attr("disabled", "disabled");
            }
        }
        if (level == 0) {
            if ($("#hdnFileNameManualAdmin").val() != "0") {
                $scope.AdminExcelGridArray = [];
                LoadSMPDataExcel($("#hdnFileNameManualAdmin").val(), level);
            }
        }
    }

    function LoadStatusOrAssign(viewType, level) {
        var SMPDataExcelModel = {
            SUNo: "0",
            ViewType: viewType,
            Level: level
        ...
        var promisePost = CIR_OperationService.postCommonService(GetSMPDataExcelUploadUrl, SMPDataExcelModel);
        promisePost.then(function (pl) {
            if (viewType == 3 && level == 1) {
                $scope.StatusArraySMPData = pl.data.result.SMPStatusOrAssignDetails;
                $scope.StatusSMPData = pl.data.result.SMPStatusOrAssignDetails[0].Id;
            }
            if (viewType == 3 && level == 2) {
                $scope.StatusArrayVerify = pl.data.result.SMPStatusOrAssignDetails;
            }
            if (viewType == 3 && level == 3)
                $scope.StatusArrayApprove = pl.data.result.SMPStatusOrAssignDetails;
            if (viewType == 3 && level == 4)
                $scope.StatusArrayPost = pl.data.result.SMPStatusOrAssignDetails;
            if (viewType == 2 && level == 1)
                $scope.AssignToArraySMPData = pl.data.result.SMPStatusOrAssignDetails;
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
    ...
    $scope.ClearSMPDataExcel = function () {
        $("#txtFileNo").removeAttr("disabled");
        $("#txtFileNameManualSMPData").removeAttr("disabled");
        $("#txtSMPDataRemarks").removeAttr("disabled");
        $("#txtFileNameManualSMPData").val("");
        $("#txtSMPDataRemarks").val("");
        LoadSMPDataExcel("0", 1);
        $scope.isEditSMPData = true;
        LoadStatusOrAssign(3, 1);
        LoadStatusOrAssign(2, 1);
    ...
    $scope.EditSMPData = function (row, level) {
        debugger
        $scope.SMPDataRowId = row.RowId;
        $scope.EditState = row.State;
        $("#hdnEditState").val(row.StateId);
        $scope.EditSite = row.Site;
        $("#hdnEditSite").val(row.SiteId);
        $scope.EditModel = row.ModelId;
        $scope.EditTurbineId = row.TurbineId;
        $("#hdnEditTurbineId").val(row.TbnId);
        $scope.EditLocation = row.Location;
        $scope.EditFunctionalLocation = row.FnLoc;
        $scope.EditFunctionalDescription = row.FunctionalDescription;
        $scope.EditPlannedDate = row.PlannedDate;
        $scope.EditCompletedOn = row.CompletedOn;
        $scope.EditValidationDate = row.ValidationDate;
        $scope.EditValidationStatus = row.ValidationStatusId;
        $scope.EditDeviationPlanVsActual = row.DeviationPlanVsActual;
        $scope.EditReportGeneratedOn = row.ReportsGeneratedOn;
        $scope.EditRecommendedActionsState = row.RecommendedActionsState;
        $scope.EditRecommendedActionsActions = row.RecommendedActions;
        $scope.EditRecommendedActionsObservations = row.RecommendedActionsObservations;
        $scope.EditRecommendedActionsWONumber = row.RecommendedActionsWONumber;
        $scope.EditRecommendedActionsCompletedDate = row.RecommendedActionsCompleted;
        $scope.EditLevelId = level;
        $("#modalSMPData").modal("show");
    }

    $scope.EditSaveSMPData = function () {
        var Data = {
            RowId: $scope.SMPDataRowId,
            TbnId: $("#hdnEditTurbineId").val(),
            StateId: $("#hdnEditState").val(),
            SiteId: $("#hdnEditSite").val(),
            ModelId: $scope.EditModel,
            Location: $scope.EditLocation,
            FnLoc: $scope.EditFunctionalLocation,
            FunctionalDescription: $scope.EditFunctionalDescription,
            PlannedDate: $scope.EditPlannedDate,
            CompletedOn: $scope.EditCompletedOn,
            ValidationDate: $scope.EditValidationDate,
            ValidationStatusId: $scope.EditValidationStatus,
            DeviationPlanVsActual: $scope.EditDeviationPlanVsActual,
            ReportsGeneratedOn: $scope.EditReportGeneratedOn,
            RecommendedActionsState: $scope.EditRecommendedActionsState,
            RecommendedActions: $scope.EditRecommendedActionsActions,
            RecommendedActionsObservations: $scope.EditRecommendedActionsObservations,
            RecommendedActionsWONumber: $scope.EditRecommendedActionsWONumber,
            RecommendedActionsCompletedDate: $scope.EditRecommendedActionsCompletedDate,
            Delflag: "0"
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataSingleDetailUrl, Data);
        promisePost.then(function (pl) {
            if (pl.data.result != null) {
                SetSuccessMessage(pl.data.result.Msg);
                if ($scope.EditLevelId == 1)
                    LoadSMPDataExcel($scope.SUNo, $scope.EditLevelId);
                if ($scope.EditLevelId == 2)
                    LoadSMPDataExcel($scope.SUNoVerify, $scope.EditLevelId);
                if ($scope.EditLevelId == 3)
                    LoadSMPDataExcel($scope.SUNoApprove, $scope.EditLevelId);
                if ($scope.EditLevelId == 4)
                    LoadSMPDataExcel($scope.SUNoPost, $scope.EditLevelId);
                $("#modalSMPData").modal("hide");
            }
            else {
                SetErrorMessage(pl.data.resul.Msg);
            }
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
    ...
    $scope.DeleteSMPData = function (row, level) {
        var Data = {
            RowId: row.RowId,
            TbnId: "0",
            StateId: "0",
            ...
            DeviationPlanVsActual: "0",
            ReportsGeneratedOn: "",
            RecommendedActionsState: "",
            RecommededActions: "",
            RecommendedActionsObservations: "",
            RecommendedActionsWONumber: "",
            RecommendedActionsCompletedDate: "",
            Delflag: "1"
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataSingleDetailUrl, Data);
        promisePost.then(function (pl) {
            if (pl.data.result != null) {
                SetSuccessMessage(pl.data.result.Msg);
                if (level == 1)
                    LoadSMPDataExcel($scope.SUNo, level);
                if (level == 2)
                    LoadSMPDataExcel($scope.SUNoVerify, level);
                $("#modalSMPData").modal("hide");
            }
            else {
                SetErrorMessage(pl.data.resul.Msg);
    ...
    $scope.LoadAssignDetailsToVerify = function () {
        var Data = {
            uploadId: $("#hdnFileNameManualVerify").val(),
            statusId: $scope.StatusVerify,
            viewType: "0"
        };

        var promisePost = CIR_OperationService.postCommonService(GetSMPAssignDetailsUrl, JSON.stringify(Data));
        promisePost.then(function (pl) {
            $scope.AssignToArrayVerify = pl.data.result;
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
    ...
    $scope.SubmitVerifyExcel = function () {
        if ($.trim($scope.RemarksVerify) == "") {
            SetErrorMessage("Ensure comments!");
            $("#txtSMPDataRemarks").focus();
            return false;
        }
        if ($scope.StatusVerify == "0" || $scope.StatusVerify == null || $scope.StatusVerify == null) {
                SetErrorMessage("Ensure Status!");
                $("#ddlStatusSMPData").focus();
                return false;
        }
        if ($scope.AssignedToVerify == "0" || $scope.AssignedToVerify == null || $scope.AssignedToVerify == null) {
            if ($scope.StatusVerify != "8") {
                SetErrorMessage("Ensure Assign To!");
                $("#ddlAssignedToSMPData").focus();
                return false;
            }
        ...
        fnLoadGif();
        var InputModel = {
            SUId: $scope.SUNoVerify,
            Remarks: $scope.RemarksVerify,
            FileName: "",
            smpData: JSON.stringify($scope.VerifyExcelGridArray),
            fileNameManual: "",
            comments: $scope.RemarksVerify,
            status: $scope.StatusVerify,
            assignTo: $scope.AssignedToVerify
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataExcelSubmit, InputModel);
        promisePost.then(function (pl) {
            $scope.Message = pl.data.result.message;
            ...
            if ($scope.Message.Clear == "True") {
                ...
                $scope.RemarksVerify = "";
                $scope.AssignedToVerify = "0";
                LoadSMPDataExcel("0", 2);
                $scope.SUNoVerify = "0";
                $scope.SUIdVerify = "0";
                $("#txtFileNameManualVerify").val("");
                $("#hdnFileNameManualVerify").val("0");
                $scope.FileNameManualVerify = "";
            }
            else {
                SetErrorMessage($scope.Message.Message);
            }
    ...
    $scope.ClearVerifyExcel = function () {
        $("#txtVerifyRemarks").removeAttr("disabled");
        $("#txtVerifyRemarks").val("");
        $scope.SUNoVerify = "0";
        $scope.SUIdVerify = "0";
        $("#txtFileNameManualVerify").val("");
        $("#hdnFileNameManualVerify").val("0");
    ...
    $scope.SubmitApproveExcel = function () {
        if ($.trim($scope.RemarksApprove) == "") {
            SetErrorMessage("Ensure comments!");
            $("#txtSMPDataRemarks").focus();
            return false;
        }
        if ($scope.StatusApprove == "0" || $scope.StatusApprove == null || $scope.StatusApprove == null) {
            SetErrorMessage("Ensure Status!");
            $("#ddlStatusApprove").focus();
            return false;
        }
        if ($scope.AssignedToApprove == "0" || $scope.AssignedToApprove == null || $scope.AssignedToApprove == null) {
            SetErrorMessage("Ensure Assign To!");
            $("#ddlAssignedToApprove").focus();
            return false;
        }
        fnLoadGif();
        var InputModel = {
            SUId: $scope.SUNoApprove,
            Remarks: $scope.RemarksApprove,
            FileName: "",
            smpData: JSON.stringify($scope.ApproveExcelGridArray),
            fileNameManual: "",
            comments: $scope.RemarksApprove,
            status: $scope.StatusApprove,
            assignTo: $scope.AssignedToApprove
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataExcelSubmit, InputModel);
        promisePost.then(function (pl) {
            $scope.Message = pl.data.result.message;
            ...
            if ($scope.Message.Clear == "True") {
                SetSuccessMessage($scope.Message.Message);
                $scope.ApproveExcelGridArray = [];
                $scope.RemarksApprove = "";
                $scope.AssignedToApprove = "0";
                LoadSMPDataExcel("0", 3);
                $scope.SUNoApprove = "0";
                $scope.SUIdApprove = "0";
                $("#txtFileNameManualApprove").val("");
                $("#hdnFileNameManualApprove").val("0");
                $scope.FileNameManualApprove = "";
            }
            else {
                SetErrorMessage($scope.Message.Message);
            }
    ...
    $scope.ClearApproveExcel = function () {
        $("#txtApproveRemarks").removeAttr("disabled");
    ...
    $scope.LoadAssignDetailsToApprove = function () {
        var Data = {
            uploadId: $("#hdnFileNameManualApprove").val(),
            statusId: $scope.StatusApprove,
            viewType: "0"
        };

        var promisePost = CIR_OperationService.postCommonService(GetSMPAssignDetailsUrl, JSON.stringify(Data));
        promisePost.then(function (pl) {
            $scope.AssignToArrayApprove = pl.data.result;
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
    ...
    $scope.SubmitPostExcel = function () {
        if ($.trim($scope.RemarksPost) == "") {
            SetErrorMessage("Ensure comments!");
            $("#txtSMPDataRemarks").focus();
            return false;
        }
        if ($scope.StatusPost == "0" || $scope.StatusPost == null) {
            SetErrorMessage("Ensure Status!");
            $("#ddlStatusPost").focus();
            return false;
        }
        ...
        var InputModel = {
            SUId: $scope.SUNoPost,
            Remarks: $scope.RemarksPost,
            FileName: "",
            smpData: JSON.stringify($scope.PostExcelGridArray),
            fileNameManual: "",
            comments: $scope.RemarksPost,
            status: $scope.StatusPost,
            assignTo: $scope.AssignedToPost
        };
        var promisePost = CIR_OperationService.postCommonService(SetSMPDataExcelSubmit, InputModel);
        promisePost.then(function (pl) {
            $scope.Message = pl.data.result.message;
            ...
            if ($scope.Message.Clear == "True") {
                ...
                $scope.RemarksPost = "";
                $scope.AssignedToPost = "0";
                LoadSMPDataExcel("0", 4);
                $scope.SUNoPost = "0";
                $scope.SUIdPost = "0";
                $("#txtFileNameManualPost").val("");
                $("#hdnFileNameManualPost").val("0");
                $scope.FileNameManualPost = "";
            }
            else {
                SetErrorMessage($scope.Message.Message);
            }
    ...
    $scope.ClearAdminExcel = function () {
        $("#txtFileNameManualAdmin").val("");
        $("#hdnFileNameManualAdmin").val("0");
        $scope.FileNameManualAdmin = "";
        LoadSMPDataExcel("0", 0);
        $("#btnCommentsAdmin").attr("disabled", "disabled");
    }

    $scope.LoadAssignDetailsToPost = function () {
        var Data = {
            uploadId: $("#hdnFileNameManualPost").val(),
            statusId: $scope.StatusPost,
            viewType: "0"
        };

        var promisePost = CIR_OperationService.postCommonService(GetSMPAssignDetailsUrl, JSON.stringify(Data));
        promisePost.then(function (pl) {
            $scope.AssignToArrayPost = pl.data.result;
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
    ...
    $(document).ready(function () {
        $('#tablist1-panel1').removeClass('responsive - tabs__list__item');
        $('#tablist1-panel2').addClass('responsive - tabs__list__item');
        $('#tablist1-panel3').addClass('responsive - tabs__list__item');
        $('#tablist1-panel4').addClass('responsive - tabs__list__item');
        $('#tablist1-panel1').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
        $("#tablist1-tab2").click(function () {
            $('#tablist1-panel2').show();
            $('#tablist1-panel3').css('display', 'none');
            $('#tablist1-panel1').css('display', 'none');
            $('#tablist1-panel4').css('display', 'none');
            $('#tablist1-panel2').removeClass('responsive - tabs__list__item');
            $('#tablist1-panel2').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel3').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel1').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel1').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item');
        });
        $("#tablist1-tab1").click(function () {
            $('#tablist1-panel1').show();
            ...
            $('#tablist1-panel3').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel2').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel2').addClass('responsive - tabs__list__item');
        });
        $("#tablist1-tab3").click(function () {
            $('#tablist1-panel3').show();
            ...
            $('#tablist1-panel1').css('display', 'none');
            $('#tablist1-panel4').css('display', 'none');
            $('#tablist1-panel3').removeClass('responsive - tabs__list__item');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel4').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item');
            $('#tablist1-panel1').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
        ...
        $("#tablist1-tab4").click(function () {
            $('#tablist1-panel4').show();
            $('#tablist1-panel1').css('display', 'none');
            $('#tablist1-panel2').css('display', 'none');
            $('#tablist1-panel3').css('display', 'none');
            $('#tablist1-panel4').removeClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel3').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel1').removeClass('responsive - tabs__list__item responsive- tabs__list__item--active');
        ...
        if ($scope.Add) {
            ...
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item');
            $('#tablist1-panel1').addClass('tab-cnt responsive-tabs__panel responsive- tabs__list__item--active responsive-tabs__panel--active responsive - tabs__list__item');
        }
            ...
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item');
            $('#tablist1-panel2').addClass('tab-cnt responsive-tabs__panel responsive- tabs__list__item--active responsive-tabs__panel--active responsive - tabs__list__item');
        }
            ...
            $('#tablist1-panel2').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('responsive - tabs__list__item');
            $('#tablist1-panel3').addClass('tab-cnt responsive-tabs__panel responsive- tabs__list__item--active responsive-tabs__panel--active responsive - tabs__list__item');
        }
            ...
            $('#tablist1-panel2').addClass('responsive - tabs__list__item');
            $('#tablist1-panel3').addClass('responsive - tabs__list__item');
            $('#tablist1-panel4').addClass('tab-cnt responsive-tabs__panel responsive- tabs__list__item--active responsive-tabs__panel--active responsive - tabs__list__item');
        }
    });

    $scope.GetComments = function (level) {
        var Data = {
            uploadId: level == 1 ? $("#hdnFileNameManualSMPData").val() == null || $("#hdnFileNameManualSMPData").val() == "0" ? $("#txtFileNo").val() : $("#hdnFileNameManualSMPData").val() : level == 2 ? $("#hdnFileNameManualVerify").val() : level == 3 ? $("#hdnFileNameManualApprove").val() : level == 4 ? $("#hdnFileNameManualPost").val() : level == 0 ? $("#hdnFileNameManualAdmin").val() : "0"
        };
        var promisePost = CIR_OperationService.postCommonService(GetCommentsUrl, JSON.stringify(Data));
        promisePost.then(function (pl) {
            $scope.CommentsGridArray = pl.data.result;
            if ($scope.CommentsGridArray.length > 0)
                $("#modalSMPDataComments").modal("show");
            else
                $("#modalSMPDataComments").modal("hide");
        }, function (err) {
            fnRemoveGif();
            SetWarningMessage("Transaction Issue. Please try again.");
            $scope.LogFile(err.config.url + " and the method is :" + err.config.method);
